#!/usr/bin/env bash
#
# <bitbar.title>OpenAI Usage (ai-usagebar)</bitbar.title>
# <bitbar.version>1.0</bitbar.version>
# <bitbar.author>Davi Ribeiro</bitbar.author>
# <bitbar.desc>Shows OpenAI/Codex plan usage in the macOS menu bar via ai-usagebar.</bitbar.desc>
...
VENDOR="openai"
LABEL="GPT "        # menu-bar prefix so this item is distinct from the Claude one
...
if [ -z "$json" ]; then
  echo "⚠️ ${LABEL}"
  echo "---"
  echo "ai-usagebar não respondeu (vendor: ${VENDOR})"
...
echo "Ocultar este item (GPT) | bash=/usr/bin/open param1=swiftbar://toggleplugin?name=openai-usage.5m.sh terminal=false"
